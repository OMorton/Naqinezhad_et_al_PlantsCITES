...
CITES_products <- CITES_Wild_Com_Exp %>% filter(Term %in% c("powder", "oil", "carvings", "dried plants", "leaves", 
...
## 74 species
(CITES_Species <- CITES_products %>% filter(Reporter.type == "E", Year %in% c(1999:2020), !grepl("spp", Taxon), !Appendix == "N",
                                       WildSource == "Yes", PurposeC == "Commercial") %>%
   group_by(Taxon) %>% slice(1) %>% select(Order, Family, Genus, Taxon) %>% ungroup()) 

CITES_products_Exp <- CITES_products %>% filter(Reporter.type == "E", Year %in% c(1999:2020), !grepl("spp", Taxon), !Appendix == "N",
                                      WildSource == "Yes", PurposeC == "Commercial") 

unique(CITES_products_Exp$Taxon)
unique (CITES_products_Exp$Term)
unique (CITES_products_Exp$Family)
...
CITES_products_Exp <- left_join(CITES_products_Exp, FL_CITES_Species, by = "Taxon")
CITES_products_Exp %>% filter(is.na(FL_year))
...
CITES_products_Exp %>% filter(Taxon %in% Del_sp) %>% summarise(n = (unique(Taxon))) ## 1 speices: Cibotium barometz
...
Species_timeframe <- CITES_products_Sum %>% group_by(Taxon, Family, Order, Exporter) %>% 
...
CITES_Exp_products <- left_join(Species_timeframe, CITES_products_Sum,
                                by = c('Taxon','Year',  'Family', 'Order', 'Exporter')) %>% 
  select(-FL_year, -Year_DEL) %>%
  rename(mass = n) %>% mutate(mass = ifelse(is.na(mass), 0, mass)) %>%
  filter(Year > 1999)

ggplot(CITES_Exp_products, aes(Year, mass)) + geom_point() + facet_wrap(~Taxon, scales = "free")

sum(CITES_Exp_products $mass)

write.csv(CITES_Exp_products, "Outputs/Data_preparation_E/Products/CITES_Exp_products_CLEAN_E.csv", na = "")
...
CITES_Exp_products <- data.table::fread("Outputs/Data_preparation_E/Products/CITES_Exp_products_CLEAN_E.csv",
                                    na.strings = "")


CITES_Exp_products <- CITES_Exp_products %>% filter(!grepl("hybrid", Taxon))
Sp_List <- CITES_Exp_products %>% group_by(Taxon) %>% tally() %>% select(Taxon) %>% as.data.frame()
CITES_Exp_products %>% filter(Year > 1999) %>% summarise(n_distinct(Taxon))
...
write.csv(df_all, "Outputs/Data_preparation_E/Products/products_IUCN_Assess_With_NA_E.csv", na = "")
df_all <- read.csv ("Outputs/Data_preparation_E/Products/products_IUCN_Assess_With_NA_E.csv", na = "")
...
CITES_IUCN_products <- left_join (CITES_Exp_products, df_new)
write.csv(CITES_IUCN_products, "Outputs/Data_preparation/Products/CITES_IUCN_Products_WITH_EXPORTER.csv", na = "")

CITES_IUCN_products_Sum <- CITES_IUCN_products %>% group_by(Taxon, Family, Order, Year, Clean_code, IUCNName) %>% tally(mass) %>%
  rename("mass" = "n")

write.csv(CITES_IUCN_products_Sum, "Outputs/Data_preparation_E/Products/CITES_IUCN_Products_E.csv", na = "")
unique(CITES_IUCN_products_Sum$Taxon)
