...
## 358,641 records
write.csv(CITES_Wild_Com, "Outputs/Data_preparation_E/Timber/CITES_Wild_Com_new_E.csv", na = "")
...
CITES_Wild_Com_Exp <- CITES_Wild_Com %>% filter(Reporter.type == "E", Year %in% c(1999:2020), 
...
unique(CITES_Timber_Terms$Family) ## 14 families
unique(CITES_Timber_Terms$Taxon) ## 53 taxa


## We have 53 species that are traded in woody terms and have units that are volumes or are masses 
...
write.csv(All_Dens, "Data/Preparation/Wood_Density_Master_E.csv")
All_Dens <-  data.table::fread("Data/Preparation/Wood_Density_Master_E.csv")
...
## 53 species
(CITES_Species <- CITES_vol %>% filter(Reporter.type == "E", Year %in% c(1999:2020), !grepl("spp", Taxon), !Appendix == "N",
                                        WildSource == "Yes", PurposeC == "Commercial") %>%
   group_by(Taxon) %>% slice(1) %>% select(Order, Family, Genus, Taxon) %>% ungroup()) 

CITES_vol_Exp <- CITES_vol %>% filter(Reporter.type == "E", Year %in% c(1999:2020), !grepl("spp", Taxon), !Appendix == "N",
                                    WildSource == "Yes", PurposeC == "Commercial") 
...
CITES_vol_Exp %>% filter(Taxon %in% Del_sp) %>% summarise(n = (unique(Taxon))) ## 0 speices
...
write.csv(CITES_vol_Exp, "Outputs/Data_preparation_E/Timber/Exp_reported_vol.csv", na = "")
...
write.csv(CITES_Exp_Timber_Vol, "Outputs/Data_preparation_E/Timber/Processed_CITES_2022.csv", na = "")
...
CITES_Exp_Timber_Vol <- data.table::fread("Outputs/Data_preparation_E/Timber/Processed_CITES_2022.csv", 
                                          na.strings = "")
...
write.csv(df_all, "Outputs/Data_preparation_E/Timber/Timber_IUCN_Assess_With_NA.csv", na = "")
df_all <- read.csv("Outputs/Data_preparation_E/Timber/Timber_IUCN_Assess_With_NA.csv", na = "")
...
write.csv(CITES_IUCN_Timber, "Outputs/Data_preparation_E/Timber/CITES_IUCN_Timber_WITH_EXPORTERS.csv", na = "")
...
write.csv(CITES_IUCN_Timber_Sum, "Outputs/Data_preparation_E/Timber/CITES_IUCN_Timber_E.csv", na = "")
