...
CITES_Wild_Com_Exp <- CITES_Wild_Com %>% filter(Reporter.type == "E", Year %in% c(1999:2020), !grepl("spp", Taxon), !grepl("Plantae", Taxon), !grepl("hybrid", Taxon), !Appendix == "N",
...
####################################################################################
CITES_live_Terms <- CITES_Wild_Com_Exp %>% filter(Term %in% c('live', "stems", "roots"), 
...
(CITES_Species <- CITES_live %>% filter (Reporter.type == "E", Year %in% c(1999:2020), !grepl("spp", Taxon), !Appendix == "N",
                                       WildSource == "Yes", PurposeC == "Commercial") %>%
   group_by(Taxon) %>% slice(1) %>% select(Order, Family, Genus, Taxon) %>% ungroup()) 

CITES_live_Exp <- CITES_live %>% filter(Reporter.type == "E", Year %in% c(1999:2020), !grepl("spp", Taxon), !Appendix == "N",
                                      WildSource == "Yes", PurposeC == "Commercial") 

unique(CITES_live_Exp$Taxon)
...
CITES_live_Exp <- left_join(CITES_live_Exp, FL_CITES_Species, by = "Taxon")
CITES_live_Exp %>% filter(is.na(FL_year))
...
CITES_live_Exp %>% filter(Taxon %in% Del_sp) %>% summarise(n = (unique(Taxon))) ## two speices (Cibotium barometz and Alsophila dregei)

ADD<- CITES_live_Exp %>% filter(Taxon %in% Add_sp) %>% summarise(n = (unique(Taxon))) ## two speices (Cibotium barometz and Alsophila dregei)
...
CITES_live_Exp <- left_join (CITES_live_Exp, CITES_Deletions) %>%
    mutate(Year_DEL = ifelse(is.na(Year_DEL), 2020, Year_DEL))

write.csv(CITES_live_Exp, "Outputs/Data_preparation_E/Live/Live_Exp_Reported_E.csv")


## Get all trades to volumes per species per year per Exporter
CITES_live_Exp_Sum <- CITES_live_Exp %>% 
  ## Importer ONLY, In our study period, No aggregate of species
  filter(Reporter.type == "E", Year %in% c(1999:2020),WildSource == "Yes", PurposeC == "Commercial", !Appendix == "N") %>% 
  ## Group and tally per species/year
  group_by(Year, Taxon, Family, Order, FL_year, Year_DEL, Exporter) %>% 
  tally(WOE) %>%
...
Species_timeframe <- CITES_live_Exp_Sum %>% group_by(Taxon, Family, Order, Exporter) %>% 
...
unique(CITES_live_Exp_Sum$Taxon)
filter(Species_timeframe, Taxon %in% c("Cibotium barometz", "Alsophila dregei")) %>% group_by(Taxon, Exporter) %>% tally()
...
CITES_live_WOE <- left_join ( Species_timeframe, CITES_live_Exp_Sum) %>% select(-FL_year, -Year_DEL)  %>%
...
write.csv(CITES_live_WOE, "Outputs/Data_preparation_E/Live/CITES_live_WOE_raw_E.csv", na = "")
...
CITES_live_WOE <- data.table::fread("Outputs/Data_preparation_E/Live/CITES_live_WOE_raw_E.csv", na.strings = "")
...
write.csv(df_all, "Outputs/Data_preparation_E/Products/live_IUCN_Assess_With_NA_E.csv", na = "")
df_all <- data.table::fread ("Outputs/Data_preparation_E/Products/live_IUCN_Assess_With_NA_E.csv", na.strings = "")
...
length(unique(CITES_IUCN_live_E$Taxon))

## Lots of species have assessments after our timeframe (e.g. 2021 and 2022) but not
#x in our timeframe (2000 - 2020)
CITES_IUCN_live_WITH_EXPORTER_E <- left_join (CITES_live_WOE, df_new)
CITES_IUCN_live_SUM <- CITES_IUCN_live_WITH_EXPORTER_E %>% 
  group_by(Taxon, Family, Order, Year, Clean_code, IUCNName) %>%
  summarise(n = sum(n))


write.csv (CITES_IUCN_live_SUM, "Outputs/Data_preparation_E/Live/CITES_IUCN_Live_E.csv", na = "")


length(unique(CITES_IUCN_live_SUM$Taxon))


write.csv (CITES_IUCN_live_WITH_EXPORTER_E, "Outputs/Data_preparation_E/Live/CITES_IUCN_Live_WITH_EXPORTER_E.csv", na = "")



##Please note that 10 species will be deleted for final analyses due to families delition in the LIve_models. So we just bring this part of the code for producing the map


CITES_IUCN_Live_WITH_EXPORTER_E <- data.table::fread ("Outputs/Data_preparation_E/Live/CITES_IUCN_Live_WITH_EXPORTER_E.csv", na.strings = "")%>%
  select(-V1)
...
CITES_IUCN_Live_WITH_EXPORTER $analysis <- if_else (CITES_IUCN_Live_WITH_EXPORTER$Family %in% c("Leguminosae", "Cucurbitaceae", "Vitaceae", "Podocarpaceae", "Pedaliaceae"), "No", "Yes")


CITES_IUCN_Live_WITH_EXPORTER <- CITES_IUCN_Live_WITH_EXPORTER %>% filter(analysis == "Yes")

write.csv (CITES_IUCN_Live_WITH_EXPORTER, "Outputs/Data_preparation/Live/CITES_IUCN_Live_WITH_EXPORTER.csv", na = "")

unique(CITES_IUCN_Live_WITH_EXPORTER$Taxon)
